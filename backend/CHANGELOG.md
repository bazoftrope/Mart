...
### Marathon Status Lifecycle

- Replaced `isActive` boolean with `status` enum: `draft`, `public`, `active`, `completed`
- **Lifecycle flow:**
  - `draft` → created by mentor, visible only to creator
  - `public` → published, visible to all users, participants can join
  - `active` → automatically set when `startDate` arrives, only participants + mentor can view
  - `completed` → automatically set when `endDate` passes, only participants + mentor can view
- **ACL updates:**
  - `listRule`/`viewRule`: hide `draft` marathons from non-owners
  - `updateRule`: allow any authenticated user to join (`participants` array)
- **New store methods:**
  - `publishMarathon(id)` — draft → public
  - `completeMarathon(id)` — active → completed
  - `updateMarathonStatusesByDate()` — auto-transition based on dates
  - `draftMarathons` getter
- **UI changes:**
  - Marathon creation defaults to `draft` status
  - Edit modal: status selector (draft/public) instead of `isActive` checkbox
  - Mentor panel: "📢 Опубликовать" button for draft marathons
  - Participant join button on MarathonView, AllMarathonsView, MarathonList
  - Visibility filtering: draft hidden from non-owners
  - Owner excluded from join button
